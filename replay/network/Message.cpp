...
// client-to-server message hashes, then server-to-client message hashes
  ...
  std::span<net::IConnection *> direct_connection_rcptf(std::vector<net::IConnection *> &, ecs::EntityId, const IMessage &msg)
  {
    if (msg.connection)
...
/* static */
  uint32_t MessageClass::initImpl(bool server, bool dbg_output_table)
  {
    ...
    for (MessageClass *cls = classLinkList; cls; cls = cls->next)
    {
      ...
      {
        EXCEPTION("Server to client message '%s'(%x) must have recipientFilter. Check message registration options.", cls->debugClassName,
               cls->classHash);
        continue;
      }
