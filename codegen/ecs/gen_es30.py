...
reserved_components_name = {"_worker_id_" : "components.getWorkerId()", "manager" : "mgr"}
...
(
  "{semanticFuncName}",
  "{moduleName}",
  ecs::EntitySystemOps({eventHandlers_str}),
  {rw_params_str},
  {ro_params_str},
  {rq_params_str},
  {no_params_str},
...
def get_es_desc(esFunction, ret):
  ...
  ret['rw_params_str'] = ('ecs::make_span({name}+{rwStart}, {rwCount})/*rw*/' if rwCount > 0 else 'ecs::empty_span()').format(name=ret['comps_desc_name'], **locals())
  ret['ro_params_str'] = ('ecs::make_span({name}+{roStart}, {roCount})/*ro*/' if roCount > 0 else 'ecs::empty_span()').format(name=ret['comps_desc_name'], **locals())
  ret['rq_params_str'] = ('ecs::make_span({name}+{rqStart}, {rqCount})/*rq*/' if rqCount > 0 else 'ecs::empty_span()').format(name=ret['comps_desc_name'], **locals())
  ret['no_params_str'] = ('ecs::make_span({name}+{noStart}, {noCount})/*no*/' if noCount > 0 else 'ecs::empty_span()').format(name=ret['comps_desc_name'], **locals())
