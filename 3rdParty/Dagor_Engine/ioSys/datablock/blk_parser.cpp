...
public:
    ...
    DataBlockParser(std::vector<char> &buf, const char *fn, bool robust_parsing,
                    DataBlock::IFileNotify *fnot) : buffer(buf),
                                                    text(&buf[0]),
                                                    curp(&buf[0]),
                ...
                if (nc == '/' && parse_comments) {
                    ...
                    if (DAGOR_UNLIKELY(DataBlock::parseCommentsAsParams)) {
                        if (wasNewlineAfterStatement || lastStatement == -1)
                            ...
                            blkRef->addNewBlock(COMMENT_POST_CPP)->addStr(
                                COMMENT_POST_CPP, mk_sub_str(cpp_comment_start, curp - 1).c_str());
                    }
                    continue;
...
#define ADD_PARAM_CHECKED(NAME, TYPE, VALUE)                                                              \
  ...
  {                                                                                                       \
    LOGE(                                                                                                 \
      "DataBlockParser: invalid value '{}' at line {} of file '{}'", (VALUE).c_str(), curLine, fileName); \
    return false;                                                                                         \
  }
...
int DataBlock::addParam(const char *name, int type, const char *value, const char *eof, int line, const char *filename,
    ...
    switch (type) {
        ...
        case TYPE_BOOL: {
            if (
                dd_stricmp(value, "yes") == 0 || dd_stricmp(value, "on") == 0 || dd_stricmp(value, "true") == 0 ||
...
bool DataBlockParser::parse(DataBlock &blk, bool isTop) {
    ...
    for (;;) {
        ...
        if (*curp == '{') {
            ...
            if (name[0] != '@' || DataBlock::parseOverridesNotApply) {
            ...
            } else if (DataBlock::parseOverridesIgnored); // do nothing
                ...
                if (!nb) {
                    LOGE("cannot find block <{}> (ordinal {}) for override in file {}", blk_name, ord,
                         blk.resolveFilename(true));
                }
                (void) ord;
                ...
                if (nid != -2) {
                    LOGE("cannot find block {} (ordinal {}) for deletion in file {}", blk_name, ord,
                         blk.resolveFilename(true));
                }
                (void) ord;
            ...
            if (name[0] != '@' || DataBlock::parseOverridesNotApply) {
                ...
                if (nid != -2) {
                    LOGE("cannot find param <{}> (ordinal {}) for override in file {}", pname, ord,
                         blk.resolveFilename(true));
                }
                (void) ord;
            } else if (strncmp(name, "@delete:", 8) == 0) {
                ...
                if (nid != -2) {
                    LOGE("cannot find param {} (ordinal {}) for deletion in file {}", pname, ord,
                         blk.resolveFilename(true));
                }
                (void) ord;
            } else if (strncmp(name, "@delete-all:", 12) == 0) {
                        ...
                        if (blk.getParamNameId(i) == nid) {
                            if (idx == 0) {
                                if (DataBlock::TYPE_STRING != blk.getParamType(i))
                                    LOGE(
                                        "different types ({} != {}) of param <{}> (ordinal {}) for override in file {}",
                                        (uint8_t) DataBlock::TYPE_STRING,
        ...
        if (label == dblk::BBF_binary_with_shared_nm || label == dblk::BBF_binary_with_shared_nm_z ||
            ...
            if (!rom_file_data.data()) {
                ...
                issue_error_load_failed(fname,
                                        fmt::format("bad ROM data for file: rom=({},{}) crd.tell()={})",
                                                    fmt::ptr(rom_file_data.data()), rom_file_data.size(),
                                                    crd.tell()).c_str());
                return false;
            }
