...
bool DataBlock::loadFromStreamBBF3(IGenLoad &crd, const int *cnv, const int *cnvEnd, const DBNameMap &strings) {
            ...
            switch (p.type) {
                case TYPE_STRING:
                    ...
                    if (s_idx < strings.strings.size()) {
                        ...
                        return false;
                    break;
                case TYPE_INT: p.v = readInt32(crd, load_le);
                    break;
                case TYPE_REAL: p.v = memcpy_cast<uint32_t, float>(readReal32(crd, load_le));
                    break;
                case TYPE_POINT3:
                case TYPE_IPOINT3:
                case TYPE_IPOINT4:
                ...
                case TYPE_E3DCOLOR: {
                    ...
                    p.v = memcpy_cast<uint32_t, E3DCOLOR>(
                        E3DCOLOR((c >> 24) & 0xFF, (c >> 16) & 0xFF, (c >> 8) & 0xFF, c & 0xFF));
                }
                break;
                case TYPE_INT64:
                    p.v = complexParamsData;
    ...
    for (int i = 0; i < blocksNum; ++i) {
        ...
        if (!addNewBlock(newNameId < 0 ? "" : shared->getName(newNameId))->
            loadFromStreamBBF3(crd, cnv, cnvEnd, strings))
            return false;
    }
