...
public:
    ...
    inline void draw_triangle_nocull(const Point2 *v, const Interpolant *interp) {
        draw_indexed_triangle_nocull(v, interp, 0, 1, 2);
    }
    ...
    inline void draw_ordered_indexed_triangle_cull(uint32_t cullMask, int top, int mid, int bot, const IntegerPoint *vi,
        ...
        int val = (bt.x >> SR_PRECISE_BITS) * (mt.y >> SR_PRECISE_BITS) - (bt.y >> SR_PRECISE_BITS) * (
                      mt.x >> SR_PRECISE_BITS); // no cull
    ...
    template<class IntegerPoint>
    inline void draw_ordered_indexed_triangle_nocull(int top, int mid, int bot, const IntegerPoint *vi,
                                                     const Interpolant *interp) {
        IPoint2 bt = IPoint2(vi[bot].x - vi[top].x, vi[bot].y - vi[top].y);
        IPoint2 mt = IPoint2(vi[mid].x - vi[top].x, vi[mid].y - vi[top].y);
        int val = (bt.x >> SR_PRECISE_BITS) * (mt.y >> SR_PRECISE_BITS) - (bt.y >> SR_PRECISE_BITS) * (
...
private:
    ...
    inline void draw_ordered_triangle(const IntegerPoint *v, const Interpolant *interp, const IPoint2 &bt,
                                      const IPoint2 &mt, int top,
                                      int mid, int bot) {
        int starty, endy;
...
public:
    ...
    void renderMeshInternal(IPoint4 *__restrict transformedVert, mat44f_cref globtm, const vec4f *__restrict vertex,
                            int vertCnt,
    ...
    void renderMesh(mat44f_cref globtm, const vec4f *__restrict vertex, int vertCnt, const Index *__restrict indices,
                    int numTri) {
