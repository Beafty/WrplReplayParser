...
// Branchless version
static bool __forceinline traceRayToTriangle(vec3f p, vec3f dir, float &min_t, float &u, float &v, vec3f vert0,
                                             vec3f edge1,
    ...
    if (v_extract_x(det) > -FLT_EPSILON && v_extract_x(det) < FLT_EPSILON)
        // exit before nan exception in win32 fpu build
        return false;
#endif
...
static bool __forceinline traceRayToTriangle(vec3f p, vec3f dir, float &min_t, vec3f vert0, vec3f edge1, vec3f edge2,
                                             bool no_cull) {
    float u, v;
    return traceRayToTriangle(p, dir, min_t, u, v, vert0, edge1, edge2, no_cull);
}

static bool __forceinline traceRayToTriangleCullCCW(vec3f p, vec3f dir, float &min_t, vec3f vert0, vec3f edge1,
                                                    vec3f edge2) {
    return traceRayToTriangle(p, dir, min_t, vert0, edge1, edge2, false);
}

static bool __forceinline traceRayToTriangleNoCull(vec3f p, vec3f dir, float &min_t, vec3f vert0, vec3f edge1,
                                                   vec3f edge2) {
    return traceRayToTriangle(p, dir, min_t, vert0, edge1, edge2, true);
}

static bool __forceinline traceRayToTriangleCullCCW(const Point3 &p, const Point3 &dir, float &min_t,
                                                    const Point3 &vert0,
                                                    const Point3 &edge1, const Point3 &edge2, float &u, float &v) {
    return traceRayToTriangle(v_ldu(&p.x), v_ldu(&dir.x), min_t, u, v, v_ldu(&vert0.x), v_ldu(&edge1.x),
                              v_ldu(&edge2.x), false);
}

static bool __forceinline traceRayToTriangleNoCull(const Point3 &p, const Point3 &dir, float &min_t,
                                                   const Point3 &vert0,
                                                   const Point3 &edge1, const Point3 &edge2, float &u, float &v) {
    return traceRayToTriangle(v_ldu(&p.x), v_ldu(&dir.x), min_t, u, v, v_ldu(&vert0.x), v_ldu(&edge1.x),
                              v_ldu(&edge2.x), true);
...
static bool __forceinline rayHitToTriangle(vec3f p, const vec3f dir, float len, vec3f vert0, vec3f edge1, vec3f edge2,
                                           bool cull) {
    float u, v;
...
static vec4f __forceinline v_dot(mat43f a, mat43f b) {
    return v_madd(a.row2, b.row2, v_madd(a.row1, b.row1, v_mul(a.row0, b.row0)));
}
...
static vec4f __forceinline traceray4TrianglesVecMask(vec3f from, vec3f dir, vec4f &min_t, mat43f p0, mat43f p1,
                                                     mat43f p2,
                                                     bool no_cull) {
    mat43f e1, e2, Ng;
...
static int __forceinline traceray4Triangles(vec3f from, vec3f dir, float &min_t, mat43f p0, mat43f p1, mat43f p2,
                                            bool no_cull,
...
traceray4TrianglesNoCull(vec3f from, vec3f dir, float &min_t, mat43f p0, mat43f p1, mat43f p2) {
    return traceray4Triangles(from, dir, min_t, p0, p1, p2, true);
}

static int __forceinline
traceray4TrianglesCullCCW(vec3f from, vec3f dir, float &min_t, mat43f p0, mat43f p1, mat43f p2) {
    return traceray4Triangles(from, dir, min_t, p0, p1, p2, false);
}

// return bit mask of triangles was hit
// min_t will be updated for each triangle independently
static int __forceinline traceray4TrianglesMask(vec3f from, vec3f dir, vec4f &min_t, mat43f p0, mat43f p1, mat43f p2,
                                                bool no_cull) {
    vec4f valid = traceray4TrianglesVecMask(from, dir, min_t, p0, p1, p2, no_cull);
    return v_signmask(valid);
}

static int __forceinline rayhit4Triangles(vec3f from, vec3f dir, float len, mat43f p0, mat43f p1, mat43f p2,
                                          bool no_cull,
                                          vec4f mask = V_CI_MASK1111) {
...
static void __forceinline v_mat44_transpose_to_mat43(vec4f r0, vec4f r1, vec4f r2, vec4f r3, vec4f &c0, vec4f &c1,
                                                     vec4f &c2) {
...
template<bool noCull>
static inline bool getTriangleHtCull(float x, float z, real &maxht, const Point3 &vert0, const Point3 &edge1,
                                     const Point3 &edge2) {
    ...
    if (noCull) {
        if (fabsf(det) < 1e-30)
            // culling: uncomment this line to make accurate culling - prevent errors on near zero det
            return false;
...
static inline vec4f getTriangle4HtCull(vec4f x, vec4f z, vec4f &maxht, const vec3f &vert0, const vec3f &edge1,
                                       const vec3f &edge2) {
    vec4f u, v;
    vec4f edge1_x = v_splat_x(edge1), edge1_z = v_splat_z(edge1), edge2_x = v_splat_x(edge2), edge2_z =
            v_splat_z(edge2);
    /// if determinant is near zero, ray lies in plane of triangle
...
static bool __forceinline getTriangleHt(float x, float z, real &maxht, const Point3 &vert0, const Point3 &edge1,
                                        const Point3 &edge2) {
...
getTrianglesMultiHtCullSOA(const vec4f *__restrict triangles, int tri_count, // array of vert0, edge1, edge2
                           mat44f *__restrict rays_soa, int rays_soa_count,
                           vec4f *__restrict v_outNorm) // array of 4 rays, v_outNorm size is rays_soa_count*4
{
    vec4f valid = v_zero();
...
static inline bool traceDownTrianglesMultiRay(const vec4f *__restrict triangles, int tri_count,
                                              // array of vert0, edge1, edge2
                                              vec4f *__restrict v_rays_pos_t, int rays_count,
                                              vec4f *__restrict v_outNorm,
                                              mat44f *__restrict tempSOA) // tempSOA is array of 4
// rays, v_rays_pos_t,
// v_norm, is array of
// [rays_count*4]
{
    for (mat44f *ray4SOA = tempSOA, *endRay4SOA = tempSOA + rays_count, *src4Ray = (mat44f *) v_rays_pos_t;
         ray4SOA != endRay4SOA;
         ray4SOA++, src4Ray++) {
...
static int __forceinline traceray4Triangles(vec4f from, vec4f dir, float &min_t, vec4f vertices[4][3], int count,
                                            bool no_cull) {
    mat43f p0, p1, p2;
    v_mat44_transpose_to_mat43(vertices[0][0], vertices[1][0], vertices[2][0], vertices[3][0], p0.row0, p0.row1,
                               p0.row2);
    v_mat44_transpose_to_mat43(vertices[0][1], vertices[1][1], vertices[2][1], vertices[3][1], p1.row0, p1.row1,
                               p1.row2);
    v_mat44_transpose_to_mat43(vertices[0][2], vertices[1][2], vertices[2][2], vertices[3][2], p2.row0, p2.row1,
                               p2.row2);
...
static int __forceinline rayhit4Triangles(vec4f from, vec4f dir, float len, vec4f vertices[4][3], int count,
                                          bool no_cull) {
    mat43f p0, p1, p2;
    v_mat44_transpose_to_mat43(vertices[0][0], vertices[1][0], vertices[2][0], vertices[3][0], p0.row0, p0.row1,
                               p0.row2);
...
static int __forceinline
traceray4TrianglesCullCCW(vec4f from, vec4f dir, float &min_t, vec4f vertices[4][3], int count) {
    return traceray4Triangles(from, dir, min_t, vertices, count, false /*no_cull*/);
}

static int __forceinline
traceray4TrianglesNoCull(vec4f from, vec4f dir, float &min_t, vec4f vertices[4][3], int count) {
...
static int __forceinline
traceray4TrianglesMask(vec4f from, vec4f dir, vec4f &min_t, vec4f vertices[4][3], bool no_cull) {
    mat43f p0, p1, p2;
    v_mat44_transpose_to_mat43(vertices[0][0], vertices[1][0], vertices[2][0], vertices[3][0], p0.row0, p0.row1,
                               p0.row2);
    v_mat44_transpose_to_mat43(vertices[0][1], vertices[1][1], vertices[2][1], vertices[3][1], p1.row0, p1.row1,
...
static vec4x2f __forceinline traceray8TrianglesVecMask(vec4x2f from, vec4x2f dir, vec4x2f &t, mat43x2f p0, mat43x2f p1,
                                                       mat43x2f p2,
                                                       bool no_cull) {
    ...
    valid = _mm256_and_ps(valid, _mm256_and_ps(_mm256_cmp_ps(det, _mm256_setzero_ps(), _CMP_GT_OQ),
                                               _mm256_and_ps(_mm256_cmp_ps(T, _mm256_setzero_ps(), _CMP_GE_OQ),
                                                             _mm256_cmp_ps(T, t, _CMP_LE_OQ))));
    t = _mm256_blendv_ps(t, T, valid);
    //_mm256_zeroupper(); will be inserted by compiler, don't add it manually
    return valid;
}

static int __forceinline traceray8Triangles(vec4x2f from, vec4x2f dir, float &min_t, mat43x2f p0, mat43x2f p1,
                                            mat43x2f p2,
                                            bool no_cull) {
...
static int __forceinline rayhit8Triangles(vec4x2f from, vec4x2f dir, float t, mat43x2f p0, mat43x2f p1, mat43x2f p2,
                                          bool no_cull) {
...
static vec4x2f __forceinline v_make_vec4x2f_mask(uint8_t bitmask) {
    vec4x2f lookup = _mm256_castsi256_ps(_mm256_set_epi32(0x3f800000 + (1 << 7), 0x3f800000 + (1 << 6),
                                                          0x3f800000 + (1 << 5),
                                                          0x3f800000 + (1 << 4), 0x3f800000 + (1 << 3),
                                                          0x3f800000 + (1 << 2), 0x3f800000 + (1 << 1),
                                                          0x3f800000 + (1 << 0)));
...
static bool __forceinline checkBoxBoundingAndTranspose(vec4f from, vec4f dir, float t, vec4f vertices[8][3],
                                                       mat43x2f &p0,
...
static int __forceinline traceray8TrianglesMask(vec4x2f from, vec4x2f dir, vec4x2f &t, mat43x2f p0, mat43x2f p1,
                                                mat43x2f p2,
                                                bool no_cull) {
    vec4x2f valid = traceray8TrianglesVecMask(from, dir, t, p0, p1, p2, no_cull);
    return _mm256_movemask_ps(valid);
...
template<bool check_bounding>
static int __forceinline traceray8TrianglesMask(vec4f from, vec4f dir, float in_t, float (&out_t)[8],
                                                vec4f vertices[8][3],
                                                bool no_cull) {
    mat43x2f p0, p1, p2;
    ...
    int ret = traceray8TrianglesMask(_mm256_set_m128(from, from), _mm256_set_m128(dir, dir), vOutT, p0, p1, p2,
                                     no_cull);
