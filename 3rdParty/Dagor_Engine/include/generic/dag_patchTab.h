...
public:
    ...
    void patch(void *base) {
        ...
        dag::Span<T>::dptr = dag::Span<T>::dcnt
                                 ? (T *) ((ptrdiff_t(dag::Span<T>::dptr) >> 32) + ptrdiff_t(base))
                                 : NULL;
#else
        dag::Span<T>::dcnt = intptr_t(dag::Span<T>::dptr) >> 32;
        dag::Span<T>::dptr = dag::Span<T>::dcnt
                                 ? (T *) ((ptrdiff_t(dag::Span<T>::dptr) & 0xFFFFFFFF) + ptrdiff_t(base))
                                 : NULL;
#endif
#else
        if (dag::Span<T>::dcnt)
...
public:
    ...
    /// Rebases already patched data address (useful for fast cloning)
    void rebase(void *newbase, const void *oldbase) {
        p = p ? (T *) (ptrdiff_t(p) + ptrdiff_t(newbase) - ptrdiff_t(oldbase)) : NULL;
    }
