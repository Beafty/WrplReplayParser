...
inline DAGOR_NOINLINE void span_index_assert([[maybe_unused]] const char *name, [[maybe_unused]] int idx,
                                             [[maybe_unused]] int dcnt,
                                             [[maybe_unused]] const void *self, [[maybe_unused]] const void *dptr,
                                             [[maybe_unused]] int szt) {
    G_ASSERT_FAIL("Index out of bounds: %s[%d] dcnt=%d this=0x%p dptr=0x%p sizeof(T)=%d", name, idx, dcnt, self, dptr,
                  szt);
}
...
public:
    ...
    /// Index operator
    inline constexpr const T &operator[](uint32_t idx) const {
        GENSLICE_VALIDATE_IDX(idx);
        return dptr[idx];
    }

    inline constexpr T &operator[](uint32_t idx) {
        GENSLICE_VALIDATE_IDX(idx);
        return dptr[idx];
    }
...
inline constexpr uint32_t data_size(const V &v) {
    return (uint32_t) v.size() * (uint32_t) sizeof(T);
}
...
inline constexpr int find_value_idx(const V &v, T1 const &val) {
    for (const U *p = v.begin(), *e = v.end(); p < e; p++)
        if (*p == val)
            return (intptr_t)(p - v.begin());
    return -1;
}
