...
namespace dag {
    ...
    public:
        ...
        template<typename T2, typename A2, bool i2, typename C2>
        friend class Vector;
        ...
        explicit Vector(size_type n,
                        const allocator_type &allocator = DAG_VECTOR_DEFAULT_ALLOCATOR) : mBeginAndAllocator(
            NULL, allocator) {
            reserve(n);
            small_vector_default_fill_n<T *, init_constructing>(mpBegin(), used() = n);
        }
        ...
        Vector(this_type &&x, const allocator_type &allocator) : mBeginAndAllocator(NULL, allocator) {
            if (internalAllocator() == x.internalAllocator()) // If allocators are equivalent...
                ...
                IF_CONSTEXPR (is_type_relocatable<value_type>::value) {
                    memcpy((void *) (mpBegin() = DoAllocate(used() = allocated() = x.size())), (const void *) x.begin(),
                           (char *) x.end() - (char *) x.begin()); //-V780
                    x.used() = 0; //x.set_capacity(0); //<- add this to release memory immediately
                } else {
                    DoInit(eastl::move_iterator<iterator>(x.begin()), eastl::move_iterator<iterator>(x.end()),
                           eastl::is_integral<iterator>());
                    x.clear(); //x.set_capacity(0); //<- replace with this to release memory immediately
                }
        }

        Vector(size_type n, const value_type &value,
               const allocator_type &allocator = DAG_VECTOR_DEFAULT_ALLOCATOR) : mBeginAndAllocator(NULL, allocator) {
            resize(n, value);
        }
        ...
        explicit Vector(Vector<value_type, A2, init_constructing, size_type> &&x,
                        const allocator_type &a) : mBeginAndAllocator(NULL, a) {
            IF_CONSTEXPR (is_type_relocatable<value_type>::value) {
                memcpy((void *) (mpBegin() = DoAllocate(used() = allocated() = x.size())), (const void *) x.begin(),
                       (char *) x.end() - (char *) x.begin()); //-V780
                x.used() = 0; //x.set_capacity(0); //<- add this to release memory immediately
        ...
        template<typename V, typename U=T, typename CB=decltype(&V::cbegin), typename CE=decltype(&V::cend),
            typename B=typename eastl::enable_if<eastl::is_same < typename V::value_type const, U const>::value, bool>
        ::type
        ,
        typename D=eastl
        ::disable_if_t<eastl::is_base_of_v<this_type, V> >
        >
        explicit Vector(const V &v, const allocator_type &a = DAG_VECTOR_DEFAULT_ALLOCATOR) : Vector(
            v.cbegin(), v.cend(), a) {
        }
        ...
        void assign(InputIterator first, InputIterator last) {
            DoAssign<InputIterator>(first, last, eastl::is_integral<InputIterator>());
        }
        ...
        inline void assign(std::initializer_list<value_type> ilist) {
            ...
            DoAssignFromIterator<InputIterator>(ilist.begin(), ilist.end(), IC());
        ...
        void shrink_to_fit() {
            if (allocated() != used()) {
                ...
                if (usingStdAllocator
                        ? DoStdExpand<value_type, usingStdAllocator>(a)
                ...
                IF_CONSTEXPR (is_type_relocatable<value_type>::value) {
                    if (usingStdAllocator
                            ? DoStdRealloc<value_type, usingStdAllocator>(a)
                            : DoRealloc<value_type, HasRealloc<allocator_type>::value>(a))
                        allocated() = a;
                } else {
                    this_type temp = this_type(eastl::move_iterator<iterator>(begin()),
                                               eastl::move_iterator<iterator>(end()), get_allocator());
                    DoSwap(temp);
                }
            }
        }
    ...
    protected:
        ...
        static constexpr size_t min_capacity_size_bytes = min_allocate_size_bytes;
        // To consider: use something like nallocx or malloc_good_size
        ...
        void DoInsertValue(const_iterator position, Args &&... args) {
            ...
            if (used() == allocated()) // if we need to expand our capacity
            {
                ...
                if (DoReallocate(nNewCap)) {
                    ...
                    ::new((void *) (pNewData + nDestPos)) value_type(eastl::forward<Args>(args)...);
                    ...
                    IF_CONSTEXPR (is_type_relocatable<value_type>::value) {
                        ...
                        pointer pNewEnd = eastl::uninitialized_move_if_noexcept(mpBegin(), destPosition, pNewData),
                                mpEnd = end();
            ...
            if (mpEnd != destPosition) {
                IF_CONSTEXPR (is_type_relocatable<value_type>::value) {
                    ...
                    ::new(static_cast<void *>(mpEnd)) value_type(eastl::move(*(mpEnd - 1)));
                    // mpEnd is uninitialized memory, so we must construct into it instead of move into it like we do with the other elements below.
                    eastl::move_backward(destPosition, mpEnd - 1, mpEnd);
                    // We need to go backward because of potential overlap issues.
                    eastl::destruct(destPosition);
                }
            }

            ::new(static_cast<void *>(destPosition)) value_type(eastl::move(value));
        ...
        void DoInsertValues(const_iterator position, size_type n, const value_type &value) {
            ...
            IF_CONSTEXPR (is_type_relocatable<value_type>::value) {
                ...
                eastl::move_backward(destPosition, mpEnd - n, mpEnd);
        ...
        void DoAssignFromIterator(RandomAccessIterator first, RandomAccessIterator last,
            ...
            IF_CONSTEXPR (eastl::is_trivially_copyable_v<value_type>)
                // Trivially copyable class implies trivial destructor
        ...
        template<typename InputIterator>
        inline void DoInsertFromIterator(const_iterator position, InputIterator first, InputIterator last,
            ...
            // n fits within existing capacity
            {
                ...
                IF_CONSTEXPR (is_type_relocatable<value_type>::value) {
                    if (mpEnd != destPosition) // branch for append-like inserts
                        memmove((void *) &destPosition[n], (const void *) destPosition,
                                (char *) mpEnd - (char *) destPosition); //-V780
                    eastl::uninitialized_copy(first, last, destPosition);
                } else if (n < nExtra)
                // If the inserted values are entirely within initialized memory (i.e. are before mpEnd)...
                {
                    eastl::uninitialized_move(mpEnd - n, mpEnd, mpEnd);
                    eastl::move_backward(destPosition, mpEnd - n, mpEnd);
                    // We need move_backward because of potential overlap issues.
        ...
        void DoAssignValues(size_type n, const value_type &value) {
            if (n > allocated()) // If n > capacity ...
            {
                size_type a = n;
                if (!DoReallocate(a)) {
                    this_type temp(n, value, internalAllocator());
                    // We have little choice but to reallocate with new memory.
                    swap(temp);
                } else {
                    size_t cnt = used();
        ...
        class HasRealloc {
            ...
            template<typename U>
            static eastl::yes_type SFINAE(decltype(&U::realloc));
        ...
        template<typename AT, typename = const size_t>
        struct get_min_expand_size {
            static constexpr size_t min_expand_size = DAG_STD_EXPAND_MIN_SIZE;
        };

        template<typename AT>
        struct get_min_expand_size<AT, decltype(AT::min_expand_size)> {
        ...
        class HasExpand {
            // Note: intentionally without exact signature match in order catch incompatible declarations of 'requestResources' in compile-time
            template<typename U>
            static eastl::yes_type SFINAE(decltype(&U::resizeInplace));
        ...
        value_type *DoReallocate(size_type &n) {
            if (!mpBegin())
                return nullptr;
            IF_CONSTEXPR (alignof(value_type) <= EA_PLATFORM_MIN_MALLOC_ALIGNMENT && is_type_relocatable<
                              value_type>::value) {
        ...
        typename eastl::disable_if<allowed, VT *>::type
        ...
        template<typename VT, bool allowed>
        typename eastl::disable_if<allowed, VT *>::type DoStdExpand(size_type &) { return nullptr; }
        ...
        template<typename VT, bool allowed>
        typename eastl::disable_if<allowed, VT *>::type DoStdRealloc(size_type &) { return nullptr; }
    ...
    Vector<T, Allocator, init_constructing, Counter>::erase(const_iterator position) {
        ...
        if (destPosition - mpBegin() < used() - 1) {
            IF_CONSTEXPR (is_type_relocatable<value_type>::value) {
                ...
                memmove((void *) destPosition, (void *) (destPosition + 1),
                        (char *) end() - (char *) (destPosition + 1));
                --used();
                return destPosition;
            } else
...
#if EASTL_ASSERT_ENABLED
        if (DAGOR_UNLIKELY(
            (first < mpBegin()) || (first > end()) || (last < mpBegin()) || (last > end()) || (last < first)))
            DAG_ASSERTF(0, "Vector::erase on invalid position %d..%d (T)=%d", first - cbegin(), last - cbegin(),
                        sizeof(T));
#endif
        if (first != last) {
            IF_CONSTEXPR (is_type_relocatable<value_type>::value) {
    ...
    template<typename T, typename Allocator, bool init_constructing, typename Counter>
    inline typename Vector<T, Allocator, init_constructing, Counter>::iterator
    Vector<T, Allocator, init_constructing, Counter>::insert(const_iterator position, size_type n,
    ...
    template<typename InputIterator>
    inline typename Vector<T, Allocator, init_constructing, Counter>::iterator
    Vector<T, Allocator, init_constructing, Counter>::insert(const_iterator position, InputIterator first,
                                                             InputIterator last) {
        const ptrdiff_t n = position - mpBegin(); // Save this because we might reallocate.
    ...
    template<typename T, typename Allocator, bool init_constructing, typename Counter>
    inline typename Vector<T, Allocator, init_constructing, Counter>::iterator
    Vector<T, Allocator, init_constructing, Counter>::insert(const_iterator position,
    ...
    inline void Vector<T, Allocator, init_constructing, Counter>::DoInit(Integer n, Integer value, eastl::true_type) {
        ...
        typedef typename eastl::remove_const<T>::type non_const_value_type;
        // If T is a const type (e.g. const int) then we need to initialize it as if it were non-const.
        eastl::uninitialized_fill_n<non_const_value_type *, value_type, Integer>(
            const_cast<non_const_value_type *>(mpBegin()), (size_type) n, value);
    }


    template<typename T, typename Allocator, bool init_constructing, typename Counter>
    template<typename InputIterator>
    inline void Vector<T, Allocator, init_constructing, Counter>::DoInit(
        InputIterator first, InputIterator last, eastl::false_type) {
        typedef typename eastl::iterator_traits<InputIterator>::iterator_category IC;
    ...
    inline bool operator==(const Vector<T, Allocator1, init1, Counter1> &a,
                           const Vector<T, Allocator2, init2, Counter2> &b) {
