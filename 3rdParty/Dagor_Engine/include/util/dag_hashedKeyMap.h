...
namespace oa_hashmap_util {
    ...
    const Val find_or(Key key, const Key *__restrict begin, const Val *__restrict vals, const Val def, uint32_t mask,
                      ValCB cb) {
        const uint32_t hashedKey = Hasher().hash(key);
        uint32_t bucketI = hashedKey & mask, bucketE = mask + 1;
        for (;;) {
...
struct HashedKeyMap {
    ...
    void emplace_new(key_t key, const val_t &val)
    // that's unsafe emplace, it requires that key hasn't been added before
    {
    ...
    void erase(key_t key, ValCB cb = ValCB()) {
        const key_t *k =
                oa_hashmap_util::find_key<key_t, EmptyKey, Hasher>(key, keys.first(), mask,
                                                                   [&](uint32_t i) { return cb(vals[i]); });
        if (k != nullptr) {
    ...
    val_t findOr(key_t key, val_t def, ValCB cb = ValCB()) const {
        return oa_hashmap_util::find_or<key_t, EmptyKey, Hasher>(key, keys.first(), vals, def, mask,
                                                                 [&](const auto &v) { return cb(v); });
    }

    template<class ValCB = oa_hashmap_util::DefaultValCB<val_t> >
    val_t *findVal(key_t key, ValCB cb = ValCB()) {
        return oa_hashmap_util::find_val<key_t, EmptyKey, Hasher>(key, keys.first(), vals, mask,
                                                                  [&](const auto &v) { return cb(v); });
    }
    ...
    bool has_key(key_t key, ValCB cb = ValCB()) const {
        return oa_hashmap_util::find_key<key_t, EmptyKey, Hasher>(key, keys.first(), mask,
                                                                  [&](uint32_t i) { return cb(vals[i]); }) !=
               nullptr;
    }
    ...
    static size_t get_capacity(size_t sz) {
    ...
    const_iterator cend() const {
        return const_iterator{*this, keys.first() + bucket_count(), keys.first() + bucket_count()};
    }
...
protected:
    ...
    void allocate(uint32_t new_capacity, key_t *&k, val_t *&v) {
        ...
        v = (val_t *) eastl::allocate_memory(internalAllocator(), size_t(new_capacity) * sizeof(val_t),
                                             EASTL_ALIGN_OF(val_t), 0);
    }
...
template<class Key, Key EmptyKey = Key(), class Hasher = oa_hashmap_util::NoHash<Key>, class allocator_type =
    EASTLAllocatorType,
...
struct HashedKeySet {
    ...
    void erase(key_t key) {
        const key_t *k = oa_hashmap_util::find_key<key_t, EmptyKey, Hasher>(
            key, keys.first(), mask, [&](uint32_t) { return true; });
        if (k != nullptr) {
            const size_t ind = k - keys.first();
            keys.first()[ind] = EmptyKey;
            --used;
    ...
    HashedKeySet(uint32_t sz = 1, const allocator_type &allocator = allocator_type("key_set")) : keys(
        nullptr, allocator) {
        if (sz) {
            sz = get_capacity(sz);
...
struct FixedCapacityHashedKeyMap {
    ...
    template<class ValCB = oa_hashmap_util::DefaultValCB<val_t> >
    val_t findOr(key_t key, val_t def, ValCB cb = ValCB()) const {
        return oa_hashmap_util::find_or<key_t, EmptyKey, Hasher>(key, keys, vals, def, mask,
                                                                 [&](const auto &v) { return cb(v); });
    }

    template<class ValCB = oa_hashmap_util::DefaultValCB<val_t> >
    val_t *findVal(key_t key, ValCB cb = ValCB()) {
        return oa_hashmap_util::find_val<key_t, EmptyKey, Hasher>(key, keys, vals, mask, [&](const auto &v) {
            return cb(v);
        });
    }
    ...
    template<class ValCB = oa_hashmap_util::DefaultValCB<val_t> >
    bool has_key(key_t key, ValCB cb = ValCB()) const {
        return oa_hashmap_util::find_key<key_t, EmptyKey, Hasher>(key, keys, mask, [&](uint32_t i) {
            return cb(vals[i]);
        }) != nullptr;
    }
