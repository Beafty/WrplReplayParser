...
public:
    ZlibLoadCB(IGenLoad &in_crd, int in_size, bool raw_inflate = false, bool fatal_errors = true) : loadCb(NULL),
        isStarted(false), isFinished(false), fatalErrors(fatal_errors) {
...
public:
    explicit ZlibDecompressSaveCB(IGenSave &in_save_cb, bool raw_inflate = false, bool fatal_errors = true);
