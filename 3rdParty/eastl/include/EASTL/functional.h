...
namespace eastl
{
	...
	///
	/// Example usage:
	///    struct TestClass { void toString() { puts("hello"); } }
	///    TestClass* pTestClassArray[3] = { ... };
	///    for_each(pTestClassArray, pTestClassArray + 3, &TestClass::toString);
	///
	...
	///    for_each(testClassArray, testClassArray + 3, &TestClass::toString);
	///
	/// Note: using conventional inlining here to avoid issues on GCC/Linux
	///
	template <typename Result, typename T>
