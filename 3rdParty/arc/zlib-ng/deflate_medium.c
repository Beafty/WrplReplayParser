...
Z_INTERNAL block_state deflate_medium(deflate_state *s, int flush) {
  ...
  for (;;) {
    ...
    if (LIKELY(!early_exit && s->lookahead > MIN_LOOKAHEAD &&
               (uint32_t) (current_match.strstart + current_match.match_length) < (s->window_size - MIN_LOOKAHEAD))) {
    ...
    if (UNLIKELY(bflush)) FLUSH_BLOCK(s, 0);
  }
  ...
  if (UNLIKELY(s->sym_next)) FLUSH_BLOCK(s, 0);
