    ...
    if (err == UNZ_OK) {
      ...
      if (central_pos64) {
        ...
        if (unzReadUInt64(&us.z_filefunc, us.filestream, &us.offset_central_dir) != UNZ_OK)
          err = UNZ_ERRNO;
      } else if ((us.gi.number_entry == UINT16_MAX) || (us.size_central_dir == UINT16_MAX) ||
                 (us.offset_central_dir == UINT32_MAX))
        err = UNZ_BADZIPFILE;
    }
  } else
    err = UNZ_ERRNO;
...
pglobal_info32->
number_entry = (uint32_t)
s->gi.
number_entry;
pglobal_info32->
size_comment = s->gi.size_comment;
pglobal_info32->
number_disk_with_CD = s->gi.number_disk_with_CD;
return UNZ_OK;
}
...
static int
unzGetCurrentFileInfoField(unzFile file, uint32_t *seek, void *field, uint16_t field_size, uint16_t size_file_field,
                           int null_terminated_field) {
  unz64_s *s = NULL;
...
static int unzGetCurrentFileInfoInternal(unzFile file, unz_file_info64 *pfile_info,
                                         ...
                                         uint16_t filename_size, void *extrafield,
...
pfile_info->
size_file_extra = file_info64.size_file_extra - file_info64.size_file_extra_internal;
pfile_info->
size_file_comment = file_info64.size_file_comment;

pfile_info->
disk_num_start = (uint16_t)
file_info64.
disk_num_start;
pfile_info->
...
{
return
unzGetCurrentFileInfoInternal(file, pfile_info, NULL, filename, filename_size,
    extrafield, extrafield_size, comment, comment_size
);
}
...
static int unzCheckCurrentFileCoherencyHeader(unz64_s *s, uint32_t *psize_variable, uint64_t *poffset_local_extrafield,
  ...
  *psize_local_extrafield = 0;
  ...
  if (unzReadUInt32(&s->z_filefunc, s->filestream, &value32) != UNZ_OK) /* size compr */
    err = UNZ_ERRNO;
  else if ((value32 != UINT32_MAX) && (err == UNZ_OK) && (value32 != s->cur_file_info.compressed_size) &&
           ((flags & 8) == 0))
    err = UNZ_BADZIPFILE;
  if (unzReadUInt32(&s->z_filefunc, s->filestream, &value32) != UNZ_OK) /* size uncompr */
    err = UNZ_ERRNO;
  else if ((value32 != UINT32_MAX) && (err == UNZ_OK) && (value32 != s->cur_file_info.uncompressed_size) &&
           ((flags & 8) == 0))
    err = UNZ_BADZIPFILE;
...
if (
unzCheckCurrentFileCoherencyHeader(s,
&size_variable, &offset_local_extrafield, &size_local_extrafield) != UNZ_OK)
...
read_buffer = (uint8_t * )ALLOC(UNZ_BUFSIZE);
pfile_in_zip_read_info->
...
pfile_in_zip_read_info->
pos_in_zipfile = s->cur_file_info_internal.offset_curfile + SIZEZIPLOCALHEADER + size_variable;
pfile_in_zip_read_info->stream.
avail_in = 0;
...
int i;
s->
pcrc_32_tab = (const z_crc_t *) get_crc_table();
init_keys(password, s
->keys, s->pcrc_32_tab);
...
s->pfile_in_zip_read->stream.
avail_out = (uint16_t)
s->pfile_in_zip_read->rest_read_compressed +
s->pfile_in_zip_read->stream.
avail_in;
}
else
...
if (bytes_not_read > 0)
memcpy(s
->pfile_in_zip_read->read_buffer, s->pfile_in_zip_read->stream.next_in, bytes_not_read);
...
total_bytes_read;
s->pfile_in_zip_read->stream.
next_in = (uint8_t *) s->pfile_in_zip_read->read_buffer;
s->pfile_in_zip_read->stream.
avail_in = (uint16_t)(bytes_not_read + total_bytes_read);
}
...
s->pfile_in_zip_read->
total_out_64 = s->pfile_in_zip_read->total_out_64 + copy;
s->pfile_in_zip_read->rest_read_uncompressed -=
copy;
s->pfile_in_zip_read->
crc32 = (uint32_t)
crc32(s
->pfile_in_zip_read->crc32,
s->pfile_in_zip_read->stream.next_out, copy);

s->pfile_in_zip_read->stream.avail_in -=
copy;
s->pfile_in_zip_read->stream.avail_out -=
copy;
s->pfile_in_zip_read->stream.next_out +=
copy;
s->pfile_in_zip_read->stream.next_in +=
copy;
s->pfile_in_zip_read->stream.total_out +=
copy;
...
s->pfile_in_zip_read->total_out_64 +=
out_bytes;
s->pfile_in_zip_read->rest_read_uncompressed -=
out_bytes;
s->pfile_in_zip_read->
...
(uint32_t)
crc32(s
->pfile_in_zip_read->crc32,buf_before, (uint32_t)out_bytes);
...
if ((err == UNZ_OK) && (pfile_info != NULL))
memcpy(pfile_info,
&s->cur_file_info, sizeof(unz_file_info64));
...
{
return
unzGoToFirstFile2(file, NULL, NULL,
0, NULL, 0, NULL, 0);
}

extern int ZEXPORT
unzGoToNextFile2(unzFile
file,
unz_file_info64 *pfile_info,
char *filename,
    uint16_t
...
{
return
unzGoToNextFile2(file, NULL, NULL,
0, NULL, 0, NULL, 0);
}
...
s->
num_file = num_file_saved;
s->
pos_in_central_dir = pos_in_central_dir_saved;
s->
cur_file_info = cur_file_info_saved;
s->
cur_file_info_internal = cur_file_info_internal_saved;
return
err;
}

extern int ZEXPORT
unzGetFilePos(unzFile
file,
...
{
file_pos->
pos_in_zip_directory = (uint32_t)
file_pos64.
pos_in_zip_directory;
file_pos->
num_of_file = (uint32_t)
file_pos64.
num_of_file;
}
return
...
if (s->pfile_in_zip_read == NULL)
return UNZ_PARAMERROR;
return (int32_t)s->pfile_in_zip_read->stream.
total_out;
}
...
if (is_within_buffer)
{
s->pfile_in_zip_read->stream.next_in += position - s->pfile_in_zip_read->
total_out_64;
s->pfile_in_zip_read->stream.
avail_in = (uInt)(stream_pos_end - position);
}
else
{
s->pfile_in_zip_read->stream.
...
s->pfile_in_zip_read->
pos_in_zipfile = s->pfile_in_zip_read->offset_local_extrafield + position;
s->pfile_in_zip_read->
rest_read_compressed = s->cur_file_info.compressed_size - position;
}
...
position;
s->pfile_in_zip_read->
total_out_64 = position;

return UNZ_OK;
}

extern int ZEXPORT
unzEndOfFile(unzFile
file)
{
unz64_s *s = NULL;
if (file == NULL)
